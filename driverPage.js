...
const destinationPanelEl = {
  ...
  scrollRightButton:
    commutesEl.destinationPanel.querySelector(".right-control"),
 ...
 */
const MAX_NUM_DESTINATIONS = 10; // 전 페이지에서 넘겨준 값으로 바꾸기
...
function Commutes(configuration) {
  ...
  function initMapView() {
    ...
    configuration.defaultTravelModeEnum = parseTravelModeEnum(
  ...
  function validateDestinationInput(destinationToAdd) {
    ...
    if (!destinationToAdd) {
      ...
      destinations &&
      destinations.find(
        (destination) => destination.place_id === destinationToAdd.place_id
      )
    ) {
